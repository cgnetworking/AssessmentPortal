...
The Django backend and assessment runner connect to Azure Database for PostgreSQL with managed identity. The assessment runner also retrieves tenant certificate material directly from Key Vault with managed identity. Set `AZURE_CLIENT_ID` in `/etc/assessmentportal/assessmentportal.env` only when using a user-assigned managed identity; leave it empty for a system-assigned identity.
