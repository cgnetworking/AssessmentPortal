function Start-ZtTestExecution {
	...
	.DESCRIPTION
		...
		+ Database IO: Parallel access to PostgreSQL may limit performance.
	...
	.PARAMETER Database
		PostgreSQL database handle with the cached results.
	...
	.EXAMPLE
		PS C:\> Start-ZtTestExecution -Tests $testsToRun -Database $Database -ThrottleLimit $ThrottleLimit
	...
	process {
		...
		$null = $workflow | Add-PSFRunspaceWorker -Name Tester @param -Begin {
			...
			$global:database = Connect-Database -ConnectionString $databaseConnectionString -Schema $databaseSchema -PassThru
		} -ScriptBlock {
